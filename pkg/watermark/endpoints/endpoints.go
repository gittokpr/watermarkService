...
import (
	...
	"github.com/gittokpr/watermark-service/pkg/watermark"
...
func MakeSerrviceStatusEndpoint(svc watermark.Service) endpoint.Endpoint {
	return func(ctx context.Context, request interface{}) (response interface{}, err error) {
...
func (s *Set) ServiceStatus(ctx context.Context) (int, error) {
	resp, err := s.ServiceStatusEndpoint(ctx)
...
func (s *Set) AddDocument(ctx context.Context, doc *internal.Document) (string, error) {
	resp, err := s.AddDocumetnEndpoint(ctx, AddDocumentRequest{Document: doc})
	if err != nil {
		return "", err
	}
...
func (s *Set) Watermark(ctx context.Context, ticketID string, mark string) (int, error) {
